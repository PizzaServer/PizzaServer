 ...
 */
public class BedrockChunk implements Chunk {
    ...
    protected BedrockChunk(World world, int x, int z, List<BedrockSubChunk> subChunks, byte[] biomeData) {
    ...
    public Block getBlock(int x, int y, int z) {
        ...
        if (blockRegistry.hasBlockType(paletteEntry.getId())) {
            ...
            block = new BedrockBlock(blockType);
    ...
    public void setBlock(BlockType blockType, Vector3i blockPosition) {
        this.setBlock(new BedrockBlock(blockType), blockPosition);
    }
    ...
    /**
     * Send the chunk blocks to a {@link BedrockPlayer}
     * It is recommended that this is done async as it can take a while to serialize.
     * @param player the {@link BedrockPlayer} to send it to
     */
    public void sendBlocksTo(BedrockPlayer player) {
        ...
        for (int subChunkIndex = 0; subChunkIndex < subChunkCount; subChunkIndex++) {
            ...
            try {
                ...
                BedrockServer.getInstance().getLogger().error("Failed to serialize subchunk (x: " + this.getX() + " z: " + this.getZ() + " index: " + subChunkCount + ")");
                return;
            }
        }
    ...
    /**
     * Send the {@link BedrockEntity}s of this chunk to a {@link BedrockPlayer}
     * This should only be called on the MAIN thread
     * @param player
     */
    public void sendEntitiesTo(BedrockPlayer player) {
    ...
    public boolean equals(Object obj) {
        if (obj instanceof BedrockChunk) {
            BedrockChunk otherChunk = (BedrockChunk)obj;
    ...
    public static class Builder {
        ...
        public Builder setWorld(BedrockWorld world) {
            this.world = world;
            return this;
        }
        ...
        public BedrockChunk build() {
            return new BedrockChunk(this.world, this.x, this.z, this.subChunks, this.biomeData);
        }

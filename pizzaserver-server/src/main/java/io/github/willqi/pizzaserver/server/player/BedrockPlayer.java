...
import io.github.willqi.pizzaserver.server.entity.BedrockLivingEntity;
...
import io.github.willqi.pizzaserver.server.player.attributes.BedrockPlayerAttributes;
...
public class BedrockPlayer extends BedrockLivingEntity implements Player {

    private final BedrockServer server;
    private final BedrockClientSession session;

    private final BedrockMinecraftVersion version;
    ...
    private final BedrockPlayerAttributes attributes = new BedrockPlayerAttributes();


    public BedrockPlayer(BedrockServer server, BedrockClientSession session, LoginPacket loginPacket) {
        this.server = server;
        ...
        if (this.hasSpawned()) {
            ...
            for (int chunkX = this.getLocation().getChunkX() - this.getChunkRadius(); chunkX <= this.getLocation().getChunkX() + this.getChunkRadius(); chunkX++) {
                for (int chunkZ = this.getLocation().getChunkZ() - this.getChunkRadius(); chunkZ <= this.getLocation().getChunkZ() + this.getChunkRadius(); chunkZ++) {
                    BedrockChunk chunk = (BedrockChunk)this.getLocation().getWorld().getChunkManager().getChunk(chunkX, chunkZ);
    ...
    public void sendChunk(int x, int z) {
        BedrockChunkManager chunkManager = (BedrockChunkManager)this.getLocation().getWorld().getChunkManager();
        if (chunkManager.isChunkLoaded(x, z)) {
            chunkManager.addChunkToPlayerQueue(this, (BedrockChunk)chunkManager.getChunk(x, z));
        } else {
            chunkManager.fetchChunk(x, z).whenComplete((chunk, exception) -> {
                if (exception != null) {
                    BedrockServer.getInstance().getLogger().error("Failed to send chunk (" + x + ", " + z + ") to player " + this.getUsername(), exception);
                    return;
                }
                chunkManager.addChunkToPlayerQueue(this, (BedrockChunk)chunk);
            });
        }
    }
    ...
    private void sendNetworkChunkPublisher() {
        ...
        packet.setCoordinates(((BedrockLocation)this.getLocation()).toVector3i());
    ...
    private void updateVisibleChunks(Location oldLocation, int oldChunkRadius) {
        Set<BedrockChunk> chunksToRemove = new HashSet<>();
            ...
            for (int chunkX = oldLocation.getChunkX() - oldChunkRadius; chunkX <= oldLocation.getChunkX() + oldChunkRadius; chunkX++) {
                for (int chunkZ = oldLocation.getChunkZ() - oldChunkRadius; chunkZ <= oldLocation.getChunkZ() + oldChunkRadius; chunkZ++) {
                    if (oldLocation.getWorld().getChunkManager().isChunkLoaded(chunkX, chunkZ)) {
                        BedrockChunk chunk = (BedrockChunk)oldLocation.getWorld().getChunkManager().getChunk(chunkX, chunkZ);
        ...
        for (BedrockChunk chunk : chunksToRemove) {
            chunk.despawnFrom(this);
        }

...
import io.github.willqi.pizzaserver.server.packs.BedrockDataPackManager;
...
import io.github.willqi.pizzaserver.server.utils.BedrockLogger;
import io.github.willqi.pizzaserver.server.utils.TimeUtils;
import io.github.willqi.pizzaserver.server.world.BedrockWorldManager;
import io.github.willqi.pizzaserver.server.world.blocks.BedrockBlockRegistry;
...
public class BedrockServer implements Server {
    ...
    private final BedrockDataPackManager dataPackManager = new BedrockDataPackManager(this);
    private final WorldManager worldManager = new BedrockWorldManager(this);
    ...
    private final BlockRegistry blockRegistry = new BedrockBlockRegistry();

    private final Logger logger = new BedrockLogger("Server");
    ...
    private BedrockServerConfig config;


    public BedrockServer(String rootDirectory) {
        INSTANCE = this;
    ...
    public void boot() {
        ...
        ((BedrockWorldManager)this.getWorldManager()).loadWorlds();
            ...
            synchronized (this.sessions) {
                ...
                while (sessions.hasNext()) {
                    ...
                    if (session.isDisconnected()) {
                        BedrockPlayer player = session.getPlayer();
                        if (player != null) {
                            player.onDisconnect();
                        }
                        sessions.remove();
    ...
    private void stop() {
        ...
        ((BedrockWorldManager)this.getWorldManager()).unloadWorlds();
    ...
    public BedrockServerConfig getConfig() {
        return this.config;
    }

    public static BedrockServer getInstance() {
        return INSTANCE;
    }
    ...
    private void setupFiles() {
        ...
        this.config = new BedrockServerConfig(config);
    ...
    private class ServerExitListener extends Thread {
        ...
        @Override
        public void run() {
            if (BedrockServer.this.running) {
                BedrockServer.this.stopByConsoleExit = true;
                BedrockServer.this.running = false;
                try {
                    synchronized (this) {
                        Thread.currentThread().wait();
                    }
                } catch (InterruptedException exception) {
                    BedrockServer.getInstance().getLogger().error("Exit listener exception", exception);
                }
            }
        }
    }
